...
const AdminViewPaymentsTable = () => {
    ...
    const searchPayments = async () => {
        try {
            const { data } = await axios.get(`${config.API_BASE_URL}/api/admin/search-payments-by-date`, {
    ...
    const fetchPaymentDetails = async () => {
        try {
            const { data } = await axios.get(`${config.API_BASE_URL}/api/admin/get-all-payments`)
            setPayments(data.payments)
            setDataStatus('')
        } catch (error) {
            setDataStatus(error.response?.data?.message || 'Error fetching payments')
    ...
    return (
        <section className='container-fluid py-4'>
            ...
            <div className='table-responsive mt-3 px-xl-5'>
                {(payments || !dataStatus) ?
...
export default AdminViewPaymentsTable
